...
TEST_CASE("Lexer", "[lexer]") {
    ...
    SECTION("Lexer Strings") {
        SECTION("Normal strings") {
            const char* testSource =
                "'test' \"test\" `test`"
            ;
            ModuleSource* src = mock_module_source(testSource);
            Lexer l(src);
        ...
        SECTION("Unterminated string exception") {
            ...
            REQUIRE_THROWS(l.tokenize(), "Encountered unexpected end of input while scanning string literal");
            delete_mocked_source(src);
        }
    }
    ...
    SECTION("Lexer Keywords") {
        ...
        REQUIRE(out[i  ].tp == tt_keyword);
        REQUIRE(out[i++].text == "sizeof");
        REQUIRE(out[i  ].tp == tt_keyword);
        REQUIRE(out[i++].text == "delete");
    ...
    SECTION("Lexer Symbols") {
        ...
        // Make sure it doesn't think ~= is a valid operator
        REQUIRE(out[i++].text == "~");
        REQUIRE(out[i  ].tp == tt_symbol);
        REQUIRE(out[i++].text == "=");
        REQUIRE(out[i  ].tp == tt_symbol);
